...
class UserInput:
    ...
    def draw(self, itemno=None):
        ...
        for idx, field in enumerate(self.fields):
            ...
            self.left.screen.addstr(
                offset, self.left.region.width - len(text), text, curses.A_BOLD
            )
    ...
    def handle_key(self, ch, y, x):
        ...
        self.right.screen.noutrefresh(
            self.left.first_visible, 0, *self.right.region.box
        )
        ...
        if ch == curses.ascii.NL:
            if self.current_field < len(self.offsets) - 1:
                ...
                self.right.screen.addstr(
                    self.offsets[self.current_field], 0, line, curses.A_ITALIC
                )
                field.value = field.from_str(line)
                curses.ungetch(curses.ascii.TAB)
                self.left.screen.noutrefresh(
                    self.left.first_visible, 0, *self.left.region.box
                )
                self.right.screen.noutrefresh(
                    self.left.first_visible, 0, *self.right.region.box
                )
