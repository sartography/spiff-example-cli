...
def configure_logging():
    ...
    task_handler.setFormatter(
        logging.Formatter(
            "%(asctime)s [%(name)s:%(levelname)s] (%(workflow_spec)s:%(task_spec)s) %(message)s"
        )
    )
    ...
    wf_handler.setFormatter(
        logging.Formatter(
            "%(asctime)s [%(name)s:%(levelname)s] (%(workflow_spec)s) %(message)s"
        )
    )
...
def add_subparsers(subparsers):
    ...
    group.add_argument(
        ...
        dest="process",
        metavar="BPMN ID",
        help="The top-level BPMN Process ID",
    )
    group.add_argument(
        "-c",
        "--collabortion",
        dest="collaboration",
        metavar="BPMN ID",
        help="The ID of the collaboration",
    )
    add_spec.add_argument(
        "-b",
        "--bpmn",
        dest="bpmn",
        nargs="+",
        metavar="FILE",
        help="BPMN files to load",
    )
    add_spec.add_argument(
        "-d", "--dmn", dest="dmn", nargs="*", metavar="FILE", help="DMN files to load"
    )
    ...
    run_wf.add_argument(
        ...
        dest="spec_id",
        metavar="SPEC ID",
        help="The ID of the spec to run",
    )
    run_wf.add_argument(
        "-a",
        "--active-ok",
    ...
    compare_spec.add_argument(
        "-o",
        "--original",
        dest="original",
        metavar="SPEC ID",
        help="The ID of the original spec",
    )
    compare_spec.add_argument(
        "-n", "--new", dest="new", metavar="SPEC ID", help="The ID of the new spec"
    )
    compare_spec.add_argument(
        "-d",
    ...
    compare_wf = subparsers.add_parser(
        "diff_workflow", help="Compare a workflow against a new spec"
    )
    compare_wf.add_argument(
        "-w",
        "--wf-id",
        dest="wf_id",
        metavar="WORKFLOW ID",
    ...
    migrate_wf.add_argument(
        "-f", "--force", dest="force", action="store_true", help="Omit validation"
    )
